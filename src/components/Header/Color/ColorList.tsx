...
const ColorList = (props: Props) => {
    ...
    const getColorItemClass = (data: ColorType) => {
        ...
        const defaultValue = `${data.color == compareColor ? classes["activiteColor"] : ""}`
        return `${classes['color-item__list']} ${isActive ? classes['activiteColor'] : ''} ${defaultValue}`

    }
    return (
        <ul className={classes["color-item"]}>
            {Color.map((data) => (
                <li key={data.id} className={getColorItemClass(data)} onClick={() => handleChangeColor(data)}>{data.name}</li>
            ))}
        </ul>
    )
}
